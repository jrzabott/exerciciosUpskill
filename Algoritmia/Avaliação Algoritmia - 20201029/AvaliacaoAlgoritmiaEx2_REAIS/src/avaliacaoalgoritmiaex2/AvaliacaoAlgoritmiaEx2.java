...
public class AvaliacaoAlgoritmiaEx2 {
    ...
    public static void main(String[] args) {
        ...
        arr = new double[len]; //ARR AGORA É INICIALIZADA COM O TAMANHO INFORMADO PELO UTILIZADOR
        for (int i = 0; i < len; i++) {
            System.out.print("Numero (" + i + "): ");
            arr[i] = ler.nextInt(); // ARMAZENAR OS len NUMEROS DESEJADOS PELO UTILIZADOR
        }
        
        double num = diferencaMaiorMenorVet(arr, arr.length); // CHAMA A FUNCAO SOLICITADA PARA ALINEA A)
        System.out.println(num);
    }

    private static double diferencaMaiorMenorVet(double[] arr, int n) {
        double max, min;
