...
public class Data implements Comparable<Data> {
    ...
    public static boolean validaDataString(String dd) throws MesInvalidoException, DiaInvalidoException {
        int ano, mes, dia;
        String temp[] = dd.split("/");
        if (temp.length != 3) {
            throw new IllegalArgumentException("Formato de Data Inválido. Por favor utilize: YYYY/MM/DD ");
        }
        ...
        try {
            ...
            throw new MesInvalidoException("Mês Inválido - "+ temp[1]);
        }
        try {
            dia = Integer.parseInt(temp[2]);
        } catch (NumberFormatException numberFormatException) {
            throw new DiaInvalidoException("Dia Inválido - " + temp[2]);
        }

        if (mes < 1 || mes > 12) {
            throw new MesInvalidoException("Mês Inválido - " + mes);
        }
        boolean diaMenor1 = dia < 1;
        boolean diaMaiorDiaMes = dia > Data.diasPorMes[mes];
        boolean VinteENoveFevereiroDeAnosBissextos = (isAnoBissexto(ano) && mes == 2 && dia == Data.diasPorMes[mes] + 1);

        if ((diaMenor1 || diaMaiorDiaMes) && !VinteENoveFevereiroDeAnosBissextos) {
